...
class Uformer(nn.Module):
    def __init__(self, img_size=128, in_chans=3,
                 ...
                 dowsample=Downsample,feature_fus=FeatureFus, upsample=Upsample, **kwargs):
        super().__init__()
        ...
        # Encoder
        self.feature_fus_0 = feature_fus(embed_dim*2, embed_dim)
        ...
        self.feature_fus_1 = feature_fus(embed_dim*4, embed_dim*2)
        ...
        self.feature_fus_2 = feature_fus(embed_dim*8, embed_dim*4)
        ...
        self.feature_fus_3 = feature_fus(embed_dim*16, embed_dim*8)
    ...
    def forward(self, x):
        ...
        deconv0 = self.decoderlayer_0(up0)
        ...
        deconv2 = self.decoderlayer_2(up2)

        up3 = self.upsample_3(deconv2)
        #deconv3 = torch.cat([up3,conv0],-1)
        #deconv3 = self.decoderlayer_3(deconv3)
        deconv3 = self.decoderlayer_3(up3)
        
        #rencoder
        rconv0 = torch.cat([y,deconv3],-1)
        rconv0 = self.feature_fus_0(rconv0)
        rconv0 = self.encoderlayer_0(rconv0)
        rpool0 = self.dowsample_0(rconv0)
        
        rconv1 = torch.cat([rpool0,deconv2],-1)
        rconv1 = self.feature_fus_1(rconv1)
        rconv1 = self.encoderlayer_1(rconv1)
        rpool1 = self.dowsample_1(rconv1)
        
        rconv2 = torch.cat([rpool1,deconv1],-1)
        rconv2 = self.feature_fus_2(rconv2)
        rconv2 = self.encoderlayer_2(rconv2)
        rpool2 = self.dowsample_2(rconv2)

        rconv3 = torch.cat([rpool2,deconv0],-1)
        rconv3 = self.feature_fus_3(rconv3)
        rconv3 = self.encoderlayer_3(rconv3)
        rpool3 = self.dowsample_3(rconv3)
        
        #rbottleneck
        rconv4 = self.conv(rpool3)
        
        #rdecoder
        rup0 = self.upsample_0(rconv4)
        rdeconv0 = self.decoderlayer_0(rup0)
        
        rup1 = self.upsample_1(rdeconv0)
        rdeconv1 = self.decoderlayer_1(rup1)

        rup2 = self.upsample_2(rdeconv1)
        rdeconv2 = self.decoderlayer_2(rup2)

        rup3 = self.upsample_3(rdeconv2)
        rdeconv3 = self.decoderlayer_3(rup3)

        # Output Projection
        y = self.output_proj(rdeconv3)
        return x + y
